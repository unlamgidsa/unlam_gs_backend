...
# import orjson
import msgspec.json as msgspec
...
class TlmyVarManager(models.Manager):
    ...
    def bulk_create(self, objs, batch_size=None, ignore_conflicts=False):
        ...
        try:
            ...
            jsonTlmyVars = msgspec.encode(ntlmyvars)
...
class TlmyVar(models.Model):
    ...
    code = models.CharField(
        "Codigo del tipo de variable",
        max_length=24,
        help_text="Codigo de la variable, se quita relacion con maestro",
        default="NoDef",
    )
    ...
    calSValue = models.CharField(
        "Valor como string de la variable de telemetria",
        default=None,
        max_length=24,
        help_text="Valor como string de la variable de telemetria",
    )

    created = models.DateTimeField(auto_now_add=True)
    tstamp = models.DateTimeField(
        default=datetime.strptime("1976-10-30 12:00:00", "%Y-%m-%d %H:%M:%S")
    )
    UnixTimeStamp = models.FloatField(
        default=0.0
    )  # db_index=True, se quita el indice, se trabaja directamente con unixtimestamp
    ...
    tlmyVarType = models.ForeignKey(
        TlmyVarType, related_name="tlmyVars", on_delete=PROTECT
    )
    tlmyRawData = models.ForeignKey(
        TlmyRawData, on_delete=PROTECT, related_name="tlmyVars", null=True
    )
    fullName = models.CharField(
        "fullname", max_length=64, help_text="fullname", default=""
    )
        ...
        if "raw" in kwargs:
            ...
            if tt.varSubType == 1:
                ...
                if tt.bitsLen >= 8:
                    ...
                    raw = unpack(
                        tt.ctype.format,
                        payload[tt.position : tt.position + tt.ctype.length],
                    )[0]
                elif tt.bitsLen == 1:
    ...
    def setValue(self, raw, dt=datetime.now(utc)):
        ...
        if self.tlmyVarType.calibrationMethod:
            if not self.tlmyVarType.calibrationLogic:
                ...
                methodToCall = getattr(
                    instance, self.tlmyVarType.calibrationMethod.aMethod
                )
        ...
        if isinstance(value, int) or isinstance(value, float):
            if (
                value >= self.tlmyVarType.limitMaxValue
                and value <= self.tlmyVarType.limitMinValue
            ):
            ...
            if self.tlmyVarType.checkoutlier:
                if (
                    value > self.tlmyVarType.outliermaxlimit
                    or value < self.tlmyVarType.outlierminlimit
                ):
                    self.outlier = True
    ...
    def toJson(self):
        return json.dumps(
            {
                "value": self.calSValue,
                "datetime": str(self.tstamp),
...
class TlmyVarInfo(models.Model):
    calSPredictedValue = models.CharField(
        "Valor predecido como string de la variable de telemetria",
        null=True,
        default=None,
        max_length=24,
        help_text="Valor predecido como string de la variable de telemetria",
    )
    genTime = models.FloatField("Time to calculate the value", default=0)

    tlmyVar = models.OneToOneField(
        TlmyVar, on_delete=models.CASCADE, primary_key=True, related_name="info"
    )
