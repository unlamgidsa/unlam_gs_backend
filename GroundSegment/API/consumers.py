...
class AsyncTlmyConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.channel_layer.group_add("RTTelemetry", self.channel_name)

        await self.accept()
        await self.send(
            text_data=json.dumps(
                {"type": "onconnect", "message": "connection accepted"}
            )
        )
    ...
    async def aOnNewtlmy(self, event):
        ...
        try:
            ...
            try:
                ...
                print(
                    "Error al intentar recuperar memoria nombre",
                    event["shareMemoryName"],
                    ex,
                )

            # print("Memoria ", event["shareMemoryName"], "consumida por ",self.clientid)
            # ntlmyVars = json.loads(jsonTlmyVars)
            ntlmyVars = msgspec.decode(sharedRTTlmyMemory.buf)
            ...
            jsonFilteredTlmyVars = msgspec.encode(filteredTlmyVars)
    ...
    async def disconnect(self, close_code):
        ...
        await self.channel_layer.group_discard("RTTelemetry", self.channel_name)
