...
frm1Dic = [
    {"name": "SinValue1", "Value": getSenoidalValue, "Ctype": "f", "encoding": "<"},
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(NB_ITERATIONS):
            ...
            try:
                ...
                self.stdout.write(
                    "Tamaño del paquete:" + str(self.utf8len(buffer.hex()))
                )
    ...
    def _init_database_(self, frm1Dic):
        ...
        for frmTvt in frm1Dic:
            ...
            frameType, created = FrameType.objects.get_or_create(
                aid=-1, description="NoFrame", satellite=sat
            )
            if created:
                frameType.save()

            ctype = CType.objects.get(format=frmTvt["encoding"] + frmTvt["Ctype"])
            um, created = UnitOfMeasurement.objects.get_or_create(
                code="N/A", description="N/A"
            )
            if created:
                um.save()

            # Siempre va a crear si se ejecuto el codigo anterior
            tlv, created = TlmyVarType.objects.get_or_create(
                code=frmTvt["name"],
